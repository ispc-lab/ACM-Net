...
class THUMOS(Dataset):
    ...
    def __init__(self, args, phase="train", sample="random"):
        ...
        self.segment_frames_num = args.segment_frames_num
        ...
        if self.phase == "train" or self.phase=="validation":
            self.rgb_feature_dir = os.path.join(self.data_dir, "features", "train", "rgb")
            self.flw_feature_dir = os.path.join(self.data_dir, "features", "train", "flow")
            self.data_list = list(open(os.path.join(self.data_dir, "split_train.txt")))
            self.data_list = [item.strip() for item in self.data_list]
            # self.data_list = os.listdir(self.rgb_feature_dir)
            # self.data_list = [os.path.splitext(item)[0] for item in self.data_list]
        else:
            self.rgb_feature_dir = os.path.join(self.data_dir, "features", "test", "rgb")
            self.flw_feature_dir = os.path.join(self.data_dir, "features", "test", "flow")
            # self.data_list = os.listdir(self.rgb_feature_dir)
            # self.data_list = [os.path.splitext(item)[0] for item in self.data_list]
            self.data_list = list(open(os.path.join(self.data_dir, "split_test.txt")))
            self.data_list = [item.strip() for item in self.data_list]

        self.data_class_list = args.class_name_lst
        self.action_class_idx_dict = {action_cls:idx for idx, action_cls in enumerate(self.data_class_list)}
    ...
    def __getitem__(self, idx):
        ...
        rgb_vid_feature = np.load(os.path.join(self.rgb_feature_dir, vid_name+".npy"))
        flw_vid_feature = np.load(os.path.join(self.flw_feature_dir, vid_name+".npy"))

        vid_len = rgb_vid_feature.shape[0]
        vid_duration = vid_len * self.segment_frames_num / self.frames_per_sec
        
        con_vid_feature = np.concatenate((rgb_vid_feature, flw_vid_feature), axis=1)
...
class ACMDataset(Dataset):
    ...
    def __init__(self, args, phase="train", sample="random"):
        ...
        if self.phase == "train":
            ...
            self.data_list = os.listdir(self.feature_dir)
            self.data_list = sorted([os.path.splitext(item)[0] for item in self.data_list])
        else:
